...
import dev.bcrick.bitwigpal.handlers.BrowserHandler;
import dev.bcrick.bitwigpal.handlers.ClipHandler;
import dev.bcrick.bitwigpal.handlers.DetailEditorHandler;
import dev.bcrick.bitwigpal.handlers.DeviceHandler;
import dev.bcrick.bitwigpal.handlers.DeviceLibrary;
import dev.bcrick.bitwigpal.handlers.GrooveHandler;
import dev.bcrick.bitwigpal.handlers.MacroHandler;
import dev.bcrick.bitwigpal.handlers.MixerHandler;
import dev.bcrick.bitwigpal.handlers.MasterDeviceHandler;
import dev.bcrick.bitwigpal.handlers.MasterHandler;
import dev.bcrick.bitwigpal.handlers.NoteHandler;
import dev.bcrick.bitwigpal.handlers.NoteInputHandler;
import dev.bcrick.bitwigpal.handlers.ProjectHandler;
import dev.bcrick.bitwigpal.handlers.SceneHandler;
import dev.bcrick.bitwigpal.handlers.SendHandler;
import dev.bcrick.bitwigpal.handlers.TrackBankManager;
import dev.bcrick.bitwigpal.handlers.TrackHandler;
import dev.bcrick.bitwigpal.handlers.TransactionHandler;
import dev.bcrick.bitwigpal.handlers.TransportHandler;
import dev.bcrick.bitwigpal.rpc.CommandQueue;
import dev.bcrick.bitwigpal.rpc.JsonRpcDispatcher;
import dev.bcrick.bitwigpal.server.ServerManager;
...
public class BitwigPalExtension extends ControllerExtension {
    ...
    // (Bitwig Settings -> Controllers -> Bitwig Pal) -> runtime discovery of a real Bitwig
    ...
    private static final String DEVICE_LIBRARY_OVERRIDE_CATEGORY = "Bitwig Pal";
    ...
    protected BitwigPalExtension(BitwigPalDefinition definition, ControllerHost host) {
    ...
    public void init() {
        ...
        CursorTrack cursorTrack = host.createCursorTrack("gig-cursor", "Bitwig Pal", 0, SCENE_COUNT, true);
        ...
        Clip cursorClip = cursorTrack.createLauncherCursorClip("bitwig-pal-clip", "Gig Clip",
        ...
        NoteInput noteInput = host.getMidiInPort(0).createNoteInput("Bitwig Pal");
        ...
        dispatcher.register("state/getTopics", params -> {
            ...
            for (String topic : dev.bcrick.bitwigpal.server.WsRpcServer.VALID_TOPICS) {
        ...
        try {
            ...
            host.println("Bitwig Pal started — HTTP on port " + DEFAULT_PORT
                + ", WebSocket on port " + (DEFAULT_PORT + 1));
        } catch (IOException e) {
            host.errorln("Failed to start Bitwig Pal servers: " + e.getMessage());
        }
    }
    ...
    public void exit() {
        ...
        host.println("Bitwig Pal stopped");
    }
    ...
    //      Bitwig Settings -> Controllers -> Bitwig Pal, if the user has set one.
    ...
    private DeviceLibrary resolveDeviceLibrary() {
        ...
        if (overrideValue != null && !overrideValue.isBlank()) {
            ...
            if (candidates.isEmpty()) {
                ...
                host.println("Bitwig Pal: multiple Bitwig device library candidates found: "
        ...
        try {
            ...
            host.println("Bitwig Pal: device library loaded: " + library.size() + " devices from "
    ...
    private List<Path> globMatch(String patternPath) {
        ...
        if (wildcardIndex < 0) {
            ...
            try {
                ...
                host.errorln("Bitwig Pal: could not construct device library path '" + patternPath
                    + "': " + e.getMessage());
                return results;
            }
        ...
        try {
            ...
            host.errorln("Bitwig Pal: could not construct device library search root '"
        ...
        if (suffixBuilder.length() > 0) {
            try {
                ...
                host.errorln("Bitwig Pal: could not construct device library suffix '"
        ...
        try (DirectoryStream<Path> stream = Files.newDirectoryStream(parent, globSegment)) {
            ...
            host.errorln("Bitwig Pal: error scanning " + parent + " for pattern '" + globSegment
                + "': " + e.getMessage());
        }
        return results;
    }
