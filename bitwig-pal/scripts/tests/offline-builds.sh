...
CLI_JAR="${PROJECT_ROOT}/build/libs/bitwig-pal-cli.jar"

# Build the CLI JAR
(cd "$REPO_ROOT" && ./gradlew :bitwig-pal:cliShadowJar -q 2>/dev/null)
TOTAL=$((TOTAL + 1))
if [ -f "$CLI_JAR" ]; then
  echo "  PASS  bitwig-pal-cli.jar exists after build"
  PASS=$((PASS + 1))
else
  echo "  FAIL  bitwig-pal-cli.jar not found after build"
  FAIL=$((FAIL + 1))
fi
...
assert_contains "CLI version output" "$VERSION" "bitwig-pal-cli"
...
(cd "$REPO_ROOT" && ./gradlew :bitwig-pal:build -q 2>/dev/null)
EXT_FILE="$HOME/Documents/Bitwig Studio/Extensions/BitwigPal.bwextension"
TOTAL=$((TOTAL + 1))
if [ -f "$EXT_FILE" ]; then
  echo "  PASS  BitwigPal.bwextension exists"
  PASS=$((PASS + 1))
else
  echo "  FAIL  BitwigPal.bwextension not found"
  FAIL=$((FAIL + 1))
fi
