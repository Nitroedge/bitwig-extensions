...
secondo/scripts/smoke-test.sh

# Offline only (schema validation, build checks)
secondo/scripts/smoke-test.sh --offline

# Online only (requires Bitwig running)
secondo/scripts/smoke-test.sh --online

# List available test scripts
secondo/scripts/smoke-test.sh --list

# Run a specific test
secondo/scripts/smoke-test.sh --only transport
```
...
The CLI reads configuration from `~/.secondo/config.json`. This file is
