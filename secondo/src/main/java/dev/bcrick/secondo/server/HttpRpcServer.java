...
public class HttpRpcServer {
    ...
    /**
     * The version reported here is {@link SecondoVersion#VERSION} and is deliberately NOT a
     * literal. It used to be one, and it was the only version string reachable over the wire, so
     * a drift between it and the extension's own {@code getVersion()} made secondo's
     * version-mismatch diagnosis report the wrong number (D-21-H).
     */
    private void handleHealth(HttpExchange exchange) throws IOException {
        addCorsHeaders(exchange);
        sendResponse(exchange, 200,
            "{\"status\":\"ok\",\"version\":\"" + SecondoVersion.VERSION + "\"}");
    }
