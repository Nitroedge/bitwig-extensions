# Secondo CLI Reference

The `gig` command-line tool controls Bitwig Studio remotely via JSON-RPC. It communicates with the Secondo controller extension running inside Bitwig over HTTP (port 8787) and WebSocket (port 8788).

**Version:** secondo-cli 0.3.0
...
flowchart LR
    ...
    HTTP -->|"transport/play"| EXT[Secondo Extension]
...
# The output JAR is at secondo/build/libs/secondo-cli.jar
# Create an alias for convenience
alias gig='java -jar /path/to/secondo-cli.jar'
```
...
|---|---|---|
| `--host <host>` | `localhost` | Server host where Secondo is running |
...
The CLI reads defaults from `~/.secondo/config.json` when present. CLI flags always override config values.
...
Send a raw JSON-RPC 2.0 request to Secondo. Useful for accessing methods not yet covered by dedicated commands.
...
| Error | Cause |
|---|---|
| `Connection refused` | Bitwig Studio is not running or Secondo extension is not active |
